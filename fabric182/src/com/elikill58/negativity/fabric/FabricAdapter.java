...
import com.elikill58.negativity.fabric.nms.Fabric_1_18_2;
...
public class FabricAdapter extends Adapter {
	...
	public FabricAdapter(FabricNegativity sn, Logger logger) {
		...
		this.versionAdapter = new Fabric_1_18_2();
	}
	...
	public boolean hasPlugin(String name) {
		try {
			return plugin.getServer().getResourceManager().getResource(new Identifier(name)) != null;
	...
	public ExternalPlugin getPlugin(String name) {
		try {
			Resource opt = plugin.getServer().getResourceManager().getResource(new Identifier(name));
			if(opt != null)
				return new FabricExternalPlugin(name, opt);
		} catch (InvalidIdentifierException | IOException ignore) {
		}
		return null;
	}
