...
public class FabricItemRegistrar extends ItemRegistrar {
	...
	private @Nullable Material findMaterial(String key) {
		...
		try {
			...
			FabricMaterial itemType = Registries.ITEM.getOrEmpty(id).map(FabricMaterial::new).orElse(null);
			if (itemType != null) {
				return itemType;
			}
			
			return Registries.BLOCK.getOrEmpty(id).map(FabricMaterial::new).orElse(null);
