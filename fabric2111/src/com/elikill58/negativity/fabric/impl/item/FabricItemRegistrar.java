...
public class FabricItemRegistrar extends ItemRegistrar {
	...
	private @Nullable Material findMaterial(String key) {
		...
		try {
			Identifier id = Identifier.of(key.toLowerCase(Locale.ROOT));
			FabricMaterial itemType = Registries.ITEM.getOptionalValue(id).map(FabricMaterial::new).orElse(null);
			if (itemType != null) {
				return itemType;
			}
			
			return Registries.BLOCK.getOptionalValue(id).map(FabricMaterial::new).orElse(null);
