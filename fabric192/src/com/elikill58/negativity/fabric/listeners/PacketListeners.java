...
public class PacketListeners {
	...
	public void onLeave(ServerPlayNetworkHandler e, MinecraftServer srv) {
		try {
			Channel channel = ReflectionUtils.getFirstWith(e.player.networkHandler.connection, ClientConnection.class, Channel.class);
	...
	private void addChannel(ServerPlayerEntity pe) {
		getOrCreateAddChannelExecutor().execute(() -> {
			Player p = FabricEntityManager.getPlayer(pe);
